...
class option_pricing:
    ...
    def __init__(self, option_type, position, time_to_maturity, spot_price, strike_price, risk_free, dividend_yield,
                 implied_vol):
    ...
    def bsm(self):
        d1 = (np.log(self.spot_price / self.strike_price) + (self.risk_free - self.dividend_yield + (
                    0.5 * self.implied_vol * self.implied_vol)) * self.time_to_maturity) / (
                         self.implied_vol * np.sqrt(self.time_to_maturity))
        d2 = d1 - (self.implied_vol * np.sqrt(self.time_to_maturity))

        if self.option_type == 'call':
            self.opt_price = self.spot_price * np.exp(-self.dividend_yield * self.time_to_maturity) * norm.cdf(
                d1) - self.strike_price * np.exp(-self.risk_free * self.time_to_maturity) * norm.cdf(d2)
        elif self.option_type == 'put':
            self.opt_price = self.strike_price * np.exp(-self.risk_free * self.time_to_maturity) * norm.cdf(
                -d2) - self.spot_price * np.exp(-self.dividend_yield * self.time_to_maturity) * norm.cdf(-d1)
...
if __name__ == "__main__":
    aa = option_pricing(option_type='call', position='long', time_to_maturity=1, spot_price=421,
                                strike_price=400, risk_free=0.0165, dividend_yield=0.02, implied_vol=0.10)

    xx = calc_implied_volatility(option_type='call',time_to_maturity=1,spot_price=421,strike_price=425,risk_free=0.0165,dividend_yield=0.02,opt_price=20)
    xx.mkt_price_to_implied_vol()
